...
#<a name="create-contentkeys-with-rest"></a>ContentKeys maken met REST
...
Media-Services kunt u nieuwe maken en leveren van gecodeerde activa. Een **ContentKey** biedt veilige toegang tot uw s **actief**. 

Wanneer u een nieuw activum (bijvoorbeeld voordat u [bestanden uploadt](media-services-rest-upload-files.md)) maakt, kunt u de volgende coderingsopties: **StorageEncrypted**, **CommonEncryptionProtected**of **EnvelopeEncryptionProtected**. 

Als u activa aan uw klanten levert, kunt u [configureren voor activa dynamisch codeert](media-services-rest-configure-asset-delivery-policy.md) met een van de volgende twee coderingen: **DynamicEnvelopeEncryption** of **DynamicCommonEncryption**.
...
1. Willekeurig genereren van een sleutel 16 bytes AES (voor gemeenschappelijke en envelop codering) of een 32-byte AES-sleutel (voor opslag-codering). 
...
3.  Codeer uw sleutel met de openbare sleutel van het x.509-certificaat. 
    ...
    Media Services .NET SDK gebruikt RSA OAEP bij het uitvoeren van de codering.  U ziet een voorbeeld in de [functie EncryptSymmetricKeyData](https://github.com/Azure/azure-sdk-for-media-services/blob/dev/src/net/Client/Common/Common.FileEncryption/EncryptionUtils.cs).
4.  Maak een controlesom (op basis van het algoritme AES PlayReady sleutel controlesom) berekend met behulp van de sleutel-id en sleutel. Zie voor meer informatie de sectie "PlayReady AES sleutel Checksum-algoritme van het document-Object Header PlayReady zich [hier](http://www.microsoft.com/playready/documents/).

    De volgende .NET functie berekent de controlesom toe met het deel van de GUID van de sleutel-id en de sleutel wissen.
...
6. De **ContentKey** entiteit koppelen aan de entiteit **activa** door de werking van de $links.

Houd er rekening mee dat voorbeelden die een AES-sleutel genereren, coderen van de sleutel en de controlesom te berekenen zijn weggelaten uit dit onderwerp. Alleen de voorbeelden die hoe tonen te communiceren met Media-Services worden geleverd.
...
Een van de waarden die u moet stellen wanneer de inhoud maken die sleutel is het type. Kies een van de volgende waarden.
