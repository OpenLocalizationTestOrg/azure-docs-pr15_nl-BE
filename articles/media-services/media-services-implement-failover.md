<properties 
    pageTitle="Implementatie van failover-scenario streaming | Microsoft Azure" 
    description="In dit onderwerp ziet u hoe u failover-scenario streaming implementeren." 
...
Dit scenario laat zien hoe inhoud (BLOB's) van het ene activum in een ander redundantie voor On-Demand streaming verwerken. In dit scenario is handig voor klanten die hun CDN foutherstel tussen twee datacenters in het geval van een storing in een van onze datacenters instellen.
In dit scenario wordt Media Services SDK voor Microsoft Azure, Microsoft Azure Media Services REST API en SDK voor Azure opslag gebruikt om aan te tonen van de volgende taken.

1. Instellen van een Media-Services-account in "A Data Center".
1. Upload een mezzanine-bestand naar een bron actief.
1. Het activum in met meerdere bits tarief MP4-bestanden coderen. 
1. Een alleen-lezen SAS locator voor het activum bron alleen toegang hebben tot de container in de opslag-account die is gekoppeld aan de bron actief maken.
1. Krijg de containernaam van de activa van de bron van de alleen-lezen SAS-locator in de vorige stap hebt gemaakt. Deze gegevens zijn nodig voor het kopiëren van BLOB's tussen opslag rekeningen (verderop in het onderwerp besproken.)
1. Maak een oorsprong locator voor de activa die door de codering taak is gemaakt. 

Vervolgens, voor het verwerken van de failover-functie:

1. Stel een account Media Services in "B Data Center".
1. Maak een leeg doel-activa in het doel van Media Services-account.
1. Maak een SAS-locator schrijven voor het doel leeg actief schrijven toegang hebben tot de container in de opslag-account die is gekoppeld aan de activa van het doel van het doel.
1. Azure opslag SDK gebruiken om te kopiëren van BLOB's (activa-bestanden) tussen de bronaccount voor opslag in "A Data Center" en de opslag van doelaccount in "B Data Center" (deze opslag-accounts zijn gekoppeld aan de activa van belang).
1. BLOB's (activa-bestanden) die zijn gekopieerd naar de doelcontainer blob met het doel activum koppelen. 
1. Maak een oorsprong locator voor het activum in "B Data Center" en geef de locator-Id die is gegenereerd voor het activum in "A Data Center". 
1. Dit geeft u de streaming URL's waarin de relatieve paden van de URL's zijn hetzelfde (alleen de base URL's verschillend zijn). 
 
Klik voor het afhandelen van eventuele storingen, kunt u een CDN op deze oorsprong locators. 

De volgende overwegingen zijn van toepassing:

- De huidige versie van Media Services SDK biedt geen ondersteuning voor het maken van een locator met een opgegeven locator-id. Voor deze taak, gebruiken we Media Services REST API.
- De huidige versie van Media Services SDK biedt geen ondersteuning voor programmatisch genereren IAssetFile informatie die een activum aan bestanden actief koppelen zou. Voor deze taak, gebruiken we de CreateFileInfos Media Services REST API. 
- Gecodeerd opslagmiddelen (AssetCreationOptions.StorageEncrypted) worden niet ondersteund voor replicatie (omdat de versleutelingssleutel verschillend voor beide accounts Media Services zijn). 
- Als u profiteren van dynamische verpakking wilt, moet u eerst minimaal één On-demand Streaming gereserveerde eenheden. Zie voor meer informatie [Dynamisch verpakking activa](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Overweeg het gebruik van de Media Services [Replicator hulpprogramma](http://replicator.codeplex.com/) als alternatief voor een failover-streaming scenario handmatig implementeren. Met dit hulpprogramma kunt u activa repliceren op twee Media Services accounts.

##<a name="prerequisites"></a>Vereisten
 
- Twee Media Services accounts in een nieuw of bestaand Azure abonnement. Zie [een Media-Services-Account te maken](media-services-portal-create-account.md).
...
In deze sectie maakt en een consoletoepassing in C#-project instellen.

1. Visual Studio gebruiken voor het maken van een nieuwe oplossing die met het project consoletoepassing in C#. Voer HandleRedundancyForOnDemandStreaming voor de naam en klik vervolgens op OK.
1. Maak de map SupportFiles op hetzelfde niveau als het bestand HandleRedundancyForOnDemandStreaming.csproj. Maak de mappen OutputFiles en MP4Files onder de map SupportFiles. Een .mp4 bestand kopiëren naar de map MP4Files (in dit voorbeeld die het bestand BigBuckBunny.mp4 wordt gebruikt). 
1. Gebruik **Nuget** toevoegen van verwijzingen naar verwante Media Services-dll's. In Visual Studio in het hoofdmenu, selecteer Extra -> bibliotheek Package Manager -> Package Manager-Console. Typ in het console venster installatie pakket windowsazure.mediaservices en druk op enter.
1. Andere referenties die vereist voor dit project zijn toevoegen: System.Configuration, System.Runtime.Serialization en System.Web.
1. Vervangen met behulp van instructies die zijn toegevoegd aan het bestand Programs.cs met de volgende standaard:
...
1. De sectie appSettings naar het .config-bestand toevoegen en bijwerken van de waarden op basis van uw sleutel Media Services en de opslag en waarden. 
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Voeg code redundantie voor On-Demand streaming worden verwerkt.



1. De volgende klasse niveau velden toevoegen aan de klasse van het programma.
...
1. De definitie van de standaard Main methode vervangen door de volgende:
...
1. Methode-definities die worden aangeroepen vanuit de belangrijkste worden hieronder toegelicht.
...
U kunt nu een manager verkeer aanvragen van de route tussen de twee datacenters en dus failover in geval van eventuele storingen.
