...
In dit artikel wordt beschreven hoe u de Service Fabric-oplossing in logboek Analytics gebruiken om u te kunnen identificeren en oplossen van problemen in uw cluster Service Fabric, door inzicht in hoe de knooppunten Fabric-Service uitvoert en hoe uw toepassingen en micro-services worden uitgevoerd.

De Service Fabric-oplossing gebruikt Azure diagnostische gegevens van uw Service Fabric VMs, door het verzamelen van deze gegevens uit uw tabellen Azure af. Logboek Analytics leest vervolgens Fabric Service framework gebeurtenissen, met inbegrip van **Betrouwbare servicegebeurtenissen**, **Gebeurtenissen Actor** **Operationele gebeurtenissen**en **ETW aangepaste gebeurtenissen**. Het dashboard Service Fabric-oplossing bevat belangrijke problemen en relevante gebeurtenissen in uw omgeving Fabric-Service.

## <a name="installing-and-configuring-the-solution"></a>Installeren en configureren van de oplossing

Volg deze drie eenvoudige stappen om te installeren en configureren van de oplossing:

1. Zorg ervoor dat de OMS werkruimte die u gebruikt gekoppeld aan de dezelfde Azure abonnement dat u gebruikt is voor het maken van de clusterbronnen meer beschikbaar, met inbegrip van opslag rekeningen. Zie [aan de slag met Analytics logboek](log-analytics-get-started.md) voor informatie over het maken van een werkruimte OMS.
2. OMS te verzamelen en weergeven van Logboeken Fabric-Service configureren.
3. Schakel de Service Fabric-oplossing in uw werkruimte.

## <a name="configure-oms-to-collect-and-view-service-fabric-logs"></a>OMS te verzamelen en weergeven van Logboeken Fabric-Service configureren
In dit gedeelte leert u OMS voor het ophalen van Service Fabric-logboeken configureren. De logboeken kunnen weergeven, analyseren en oplossen van problemen in de toepassingen en services die worden uitgevoerd in het cluster, of in het cluster met behulp van de portal OMS.

>[AZURE.NOTE] De extensie Azure diagnostische gegevens moet worden geconfigureerd om de logboeken uploaden naar opslag tabellen die overeenkomen met wat OMS zoekt. Zie [Logboeken met Azure diagnostische gegevens verzamelen](../service-fabric/service-fabric-diagnostics-how-to-setup-wad.md) voor meer informatie over het verzamelen van Logboeken. De configuratie instellingen in dit artikel voorbeelden moet u wat de namen van de tabellen voor opslag. Zodra de diagnose is ingesteld op het cluster en Logboeken is geüpload naar een opslag, is de volgende stap OMS voor het verzamelen van deze logboeken configureren.

Controleer de sectie **EtwEventSourceProviderConfiguration** in het bestand **template.json** posten voor de nieuwe EventSources toevoegen voordat u de configuratie-update toepassen door het uitvoeren van **deploy.ps1**bij te werken. De tabel voor de upload is gelijk aan (ETWEventTable). Op het moment, OMS alleen toepassingsgebeurtenissen ETW gelezen uit die tabel. Echter ondersteuning voor aangepaste tabellen voor ETW in ontwikkeling is.

De volgende hulpprogramma's worden gebruikt voor het uitvoeren van sommige van de bewerkingen in deze sectie:

-   Azure PowerShell
-   [Operations Management Suite](http://www.microsoft.com/oms)

### <a name="configure-an-oms-workspace-to-show-the-cluster-logs"></a>Een werkruimte OMS zodat de cluster-logboeken configureren

Als u een werkruimte OMS zoals hierboven beschreven hebt gemaakt, wordt de volgende stap is de werkruimte halen de logboeken van de tabellen Azure opslag waar ze worden geüpload uit het cluster door de uitbreiding van diagnostische gegevens configureren. Om dit te doen, voer het volgende PowerShell-script:
...
Nadat u hebt geconfigureerd de OMS werkruimte te lezen uit de Azure tabellen in uw account voor opslag, meld u aan bij de Azure portal en selecteert u de werkruimte OMS uit **Alle bronnen**. Wanneer geselecteerd, ziet u het nummer van de opslag van logboeken aan die werkruimte OMS verbonden rekening. Selecteer de tegel **opslag account logboeken** en controleer of in de lijst met Logboeken voor opslag account dat uw opslag-account is verbonden met die werkruimte OMS:

![Logboeken voor opslag account](./media/log-analytics-service-fabric/sf1.png)

## <a name="enable-the-service-fabric-solution"></a>De oplossing Fabric-Service inschakelen
Gebruik het volgende script de oplossing toevoegen aan uw werkruimte OMS. Voer het script in PowerShell, Azure-abonnement dat is gekoppeld aan de werkruimte voor OMS die u wilt inschakelen van de Service Fabric-oplossing in gebruik.
...
Nadat de oplossing is ingeschakeld, wordt de tegel Fabric-Service toegevoegd aan de pagina overzicht OMS met een overzicht van belangrijke kwesties als de runAsync storingen en annuleringen die hebben plaatsgevonden in de afgelopen 24 uur.

![Service-structuur tegel](./media/log-analytics-service-fabric/sf2.png)

### <a name="view-service-fabric-events"></a>Service Fabric-gebeurtenissen weergeven

Klik op de tegel **Fabric Service** om te openen het dashboard Fabric-Service. Het dashboard bestaat uit de kolommen in de volgende tabel. Elke kolom bevat de bovenste tien gebeurtenissen per aantal overeenkomen met de criteria van de kolom voor het opgegeven bereik. U kunt een zoekfunctie waarmee u de hele lijst door te klikken op **alle** onderaan rechts van elke kolom of door te klikken op de kolomkop kunt uitvoeren.
...
>[AZURE.NOTE] Door te klikken op de **gegevens op basis van de laatste 7 dagen** aan de bovenkant van het dashboard kunt u het bereik van deze gebeurtenissen in de Service Fabric-oplossing. U kunt ook gebeurtenissen die worden gegenereerd in de laatste 7 dagen, 1 dag of 6 uur weergeven. Of u kunt **aangepaste** geeft u een aangepast datumbereik selecteren.


## <a name="troubleshoot-your-service-fabric-and-oms-configuration"></a>Problemen oplossen met de configuratie van de Service weefsel en OMS

Als u nodig hebt om te controleren of de configuratie van uw Kantoorbeheersysteem omdat u om gebeurtenisgegevens te bekijken in de OMS, gebruikt u het onderstaande script. De configuratie van diagnostische Service Fabric, controles voor gegevens die worden weggeschreven naar de tabellen, worden gelezen en er wordt gecontroleerd of de OMS is geconfigureerd om te lezen uit de tabellen.
