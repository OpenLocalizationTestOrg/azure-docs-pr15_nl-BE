<properties
   pageTitle="Implementatie van meerdere NIC VMs in Resource Manager met behulp van de CLI Azure | Microsoft Azure"
   description="Informatie over het implementeren van meerdere NIC VMs met behulp van de CLI Azure in Resource Manager"
...
#<a name="deploy-multi-nic-vms-using-the-azure-cli"></a>Meerdere NIC VMs met behulp van de CLI Azure implementeren
...
U kunt het volledige bash script gebruikt downloaden [hier](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/IaaS-Story/11-MultiNIC/arm/virtual-network-deploy-multinic-arm-cli.sh). Volg de onderstaande stappen om te wijzigen in uw omgeving werkt met het script.
...
3. Ophalen van de ID voor de `BackEnd` subnet waar het VMs worden gemaakt. U moet dit doen omdat de NIC's worden gekoppeld aan dit subnet in een andere bronnengroep bevinden.
...
4. Ophalen van de ID voor de `NSG-RemoteAccess` NSG. U moet dit doen omdat de NIC's worden gekoppeld aan deze NSG in een andere bronnengroep bevinden zijn.
...
1. Maak een nieuwe bronnengroep voor alle bronnen van de back-end. Let op het gebruik van de `$backendRGName` variabele voor de naam van de resource, en `$location` voor de regio Azure.
...
3. Maak een beschikbaarheid voor de VMs instellen.
...
2. Voor elke VM, maakt u een NIC voor toegang tot de database.
...
3. Voor elke VM, maakt u een NIC voor externe toegang. Aankondiging van de `--network-security-group` parameter gebruikt voor het koppelen van de NIC naar een NSG.
...
5. Voor elke VM, twee gegevensschijven maken en beëindig de lus met de `done` opdracht.
