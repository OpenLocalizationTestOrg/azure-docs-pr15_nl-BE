<properties
    pageTitle="Azure Active Directory reporting controle monsters API | Microsoft Azure"
...
# <a name="azure-active-directory-reporting-audit-api-samples"></a>Azure Active Directory reporting monsters voor controle-API
...
Het bereik van dit onderwerp is te voorzien u voorbeeldcode voor het **controleren van de API**.
...
App-verificatie werkt niet als de huurder in de EU-regio. Gebruik de gebruikersverificatie voor het openen van de API van de controle als een tijdelijke oplossing totdat het probleem worden opgelost. 
...
### <a name="executing-the-powershell-script"></a>De PowerShell-script uitvoeren
...
Het script wordt uitvoer uit het controleverslag in JSON-indeling. Ook is er een `audit.json` bestand met hetzelfde resultaat. U kunt experimenteren met het aanpassen van het script om gegevens uit andere lijsten en commentaar van de output-indelingen die u niet nodig hebt.
...
- Wilt u de voorbeelden in dit onderwerp aanpassen? Bekijk de [Azure Active Directory API reference controleren](active-directory-reporting-api-audit-reference.md). 
